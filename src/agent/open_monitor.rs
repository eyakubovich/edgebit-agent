...
impl<'cb> CommBuffer<'cb> {
    fn load<'a, F>(map: CommBufferMap<'a>, cb: F) -> Result<Self>
    where
...
impl BpfProbes {
    ...
    fn load_internal(use_ring_buf: bool, with_optional_probes: bool) -> Result<Self> {
        ...
        let mut skel = open_skel.load().map_err(|err| LoadError(err))?;

        skel.attach().map_err(|err| LoadError(err))?;

        return Ok(Self { skel, use_ring_buf });
    }
